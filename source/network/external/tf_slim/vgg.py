...
def vgg_19_conv(inputs,
  ...
  Returns:
    net: the output of the logits layer.
    end_points: a dict of tensors with intermediate activations.
  """
  with tf.variable_scope(scope, 'vgg_19', [inputs], reuse=tf.AUTO_REUSE) as sc:
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected, slim.max_pool2d],
      ...
      end_points = slim.utils.convert_collection_to_dict(end_points_collection)
      return net, end_points
vgg_19_conv.default_image_size = 224


def vgg_16_ssd512(inputs,
           is_training=True,
           dropout_keep_prob=0.5,
           spatial_squeeze=True,
           scope='vgg_16',
  ...
  with tf.variable_scope(scope, 'vgg_16', [inputs], reuse=tf.AUTO_REUSE) as sc:
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected, slim.max_pool2d],
      ...
      end_points = slim.utils.convert_collection_to_dict(end_points_collection)
      return net, end_points
vgg_16_ssd512.default_image_size = 512


def vgg_19_ssd512(inputs,
                is_training=True,
                dropout_keep_prob=0.5,
                spatial_squeeze=True,
                scope='vgg_19',
  ...
  with tf.variable_scope(scope, 'vgg_19', [inputs], reuse=tf.AUTO_REUSE) as sc:
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected, slim.max_pool2d],
      ...
      end_points = slim.utils.convert_collection_to_dict(end_points_collection)
      return net, end_points
vgg_19_ssd512.default_image_size = 512
