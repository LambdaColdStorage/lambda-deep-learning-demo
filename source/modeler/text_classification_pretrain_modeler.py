...
class TextClassificationPretrainModeler(Modeler):
  def __init__(self, config, net):
    super(TextClassificationPretrainModeler, self).__init__(config, net)
    self.grad_clip = 5.
  ...
  def get_dataset_info(self, inputter):
    ...
    self.embedding = inputter.get_embedding()
...
def build(config, net):
  return TextClassificationPretrainModeler(config, net)
